...
class AriaDataset(Dataset):
    """PyTorch Dataset for a single stream (e.g., RGB), index-based access."""
...
class AriaMultistreamDataset(torch.utils.data.Dataset):
    """PyTorch Dataset for synchronized access to multiple streams."""
    def __init__(self, provider, stream_ids, timestamps_ns, transform=None):
        ...
        if not isinstance(self.stream_ids, (list, tuple)):
            raise ValueError("stream_ids must be a list or tuple of stream IDs")
